    ...
    pool.query(queries.getClienteByCpf, [cpf], (error, results) => {
        if (results.rows.length === 0) {
            ...
            pool.query(queries.removeCliente, [cpf], (error, results) => {
                return res.status(200).json({ msg: 'Cliente removido com sucesso. ' });
            })
        }
    });
}
...
const updateCliente = (req, res) => {
    ...
    const { nome, telefone, numero, cidade, bairro, cep, rua, estado, email, complemento, nome_pet, pet_id, raca_pet } = req.body;
    ...
    pool.query(queries.getClienteByCpf, [cpf], (error, results) => {
        ...
        if (results.rows.length === 0) {
            ...
            pool.query(queries.updateCliente, [nome, telefone, numero, cidade, bairro, cep, rua, estado, email, complemento, cpf], (error, results) => {
                ...
                pool.query(queries.updatePet, [nome_pet, raca_pet, pet_id], (error, results) => {

                    return res.status(200).json({ msg: 'Cliente e pet atualizados com sucesso!' });
                })
            })
        }
